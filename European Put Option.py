...
# Pricing European Put Option through simulation
...
# Option Return at t = T: max( 0, K - S(T) ), where K is a Strike.
...
def expected_value( N, S_0, r, vol, T, K):
    ...
    for i in range(N):
        payment = K - simulate_path( S_0, r, vol, T )
...
def black_scholes( S_0, r, vol, T, K ):
    ...
    value = K * math.exp( -r * T ) * norm.cdf( -d2 ) - S_0 * norm.cdf( -d1 ) 
...
compare_values_for_N( 1, 0.02, 0.1, 2, 1.3 )
